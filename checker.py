...
def score_password(password: str) -> dict:
    ...
    if not recs:
        recs.append(
            "Пароль выглядит хорошо, но проверьте уникальность и храните его в менеджере паролей."
        )
    results["recommendations"] = recs

    print(f"[DEBUG] score_password: итоговые баллы={results['score']}, уровень={results['level']}, энтропия={results['entropy']:.1f}")
    return results
...
def generate_password(
    ...
    print(
        f"[DEBUG] generate_password: длина={length}, lower={use_lower}, upper={use_upper}, digits={use_digits}, symbols={use_symbols}"
    )
    if length < 4:
        length = 4
    if length > 128:
